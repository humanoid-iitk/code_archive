...
pub=rospy.Publisher('commands' , String  , queue_size = 4)
...
while True:
	...
	_ , cntLine , _ = cv2.findContours(lineroi, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
	...
	if len(cntArrowBox) > 0:
		...
		if BoxArea > 1000:
			...
			dist = mp().GetYa(rows-y , 55*np.pi/180)
			print(dist)
			if dist < 25
	...
	if len(cntLine) > 0 and BoxArea < 1000:
